...
- Threshold weapon tags are sign-sensitive. New syntax such as `TargetShield(SF>10%)`, `AvoidShield(SF>10%)`, `TargetShield(TF>10%)`, `AvoidShield(TF>10%)`, and `BurstPD(SF>10%)` should parse `10%` as `0.10`, not `0.90`. Legacy aliases may still require inversion depending on their old semantics.
