...
- The active implementation focus is now the remaining naming/logic review wave: add `PrioBig`, review/canonicalize `PrioPD -> PrioSmall`, review/canonicalize `BigShip/SmallShip -> TargetBig/TargetSmall`, review `TargetPhase -> PrioPhase` only if semantics match prioritization, then do README tag-table synchronization/examples and remaining text consistency.
